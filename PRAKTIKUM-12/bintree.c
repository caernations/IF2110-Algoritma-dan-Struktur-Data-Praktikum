...
int hitungNode(BinTree root) {
    ...
    int leftCount = hitungNode(LEFT(root));
    int rightCount = hitungNode(RIGHT(root));
...
void sumbebek(BinTree root, int n, int currentSum, int currentProduct, int *hasil)
{
    if (root == NULL)
    {
        return;
    }
    currentSum += ROOT(root);
    currentProduct = (currentProduct * ROOT(root)) % 10000;
    if (currentSum == n)
    {
        *hasil = (*hasil + currentProduct) % 10000;
    }
    if (currentSum > n)
    {
        return;
    }
    sumbebek(LEFT(root), n, currentSum, currentProduct, hasil);
    sumbebek(RIGHT(root), n, currentSum, currentProduct, hasil);
}

int hitungBebek(BinTree root, int n)
{
    int hasil = 0;
    sumbebek(root, n, 0, 1, &hasil);
    return hasil;
}
